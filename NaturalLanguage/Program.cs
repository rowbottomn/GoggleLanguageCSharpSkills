/*
Nathan Rowbottom
Uses Googles Natural Language Sentiment Analysis to analysis text for positivity
1.0 Tinked lightly with tutorial 
2.0 Created Paragraph class to gether sentiment
3.0 Added sentence by sentence analysis and paragraph averaging
4.0 Added file reading
*/
...
namespace NaturalLanguageApiDemo
{
    class Program
    {
        ...
        {
            Paragraph [] paragraphs = new Paragraph[1];//I originally made the paragraph class to handle individually each paragraph but it is
           
            try
            {   // Open the text file using a stream reader.
                using (StreamReader sr = new StreamReader("Sermon.txt"))
                {
                    String line = sr.ReadToEnd();
                    Console.WriteLine(line);
                     paragraphs[0] = new Paragraph(line);
                }
            }
            catch (IOException e)
            {
